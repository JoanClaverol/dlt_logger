...
def _create_job_logs_resource():
    """Create job_logs resource with dynamic table name from config."""
    config = get_config()
    ...
    def job_logs_resource_impl(
        ...
        with duckdb.connect(db_path, read_only=True) as conn:
            ...
            SELECT
                *
            FROM {dataset_name}.{config.table_name}
            """
    ...
    return job_logs_resource_impl


def job_logs_resource(db_path: str, dataset_name: str, batch_size: int = 10000):
    """Create and return the dynamically configured job_logs resource."""
    resource_func = _create_job_logs_resource()
    return resource_func(db_path, dataset_name, batch_size)
