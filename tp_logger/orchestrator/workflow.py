...
from ..logging import TPLogger, setup_logging, get_logger
from ..utils import (
...
class WorkflowManager:
    ...
    def step_4_transfer_to_athena(self) -> bool:
        ...
        try:
            ...
            if not self.config.athena_destination:
                self.logger.info("ℹ️  Athena transfer skipped (athena_destination=False)")
                return True
            ...
            self.logger.info(f"S3 Staging Bucket: {self.config.athena_s3_staging_bucket}")
            
            # Debug current config state
            current_config = get_config()
            self.logger.info(f"[WORKFLOW] Current global config - athena_destination: {current_config.athena_destination}")
            self.logger.info(f"[WORKFLOW] Current global config - db_path: {current_config.db_path}")
            self.logger.info(f"[WORKFLOW] Current global config - dataset_name: {current_config.dataset_name}")
            
            # Check if database exists
            import os
            if os.path.exists(current_config.db_path):
                self.logger.info(f"[WORKFLOW] Database file exists: {current_config.db_path} ({os.path.getsize(current_config.db_path)} bytes)")
            else:
                self.logger.error(f"[WORKFLOW] Database file missing: {current_config.db_path}")
                return False
            ...
            self.logger.error(f"❌ Athena transfer failed: {type(e).__name__}: {str(e)}")
    ...
    def run_complete_workflow(self, sample_log_count: int = 10) -> Dict[str, Any]:
        ...
        results["overall_success"] = all(step["success"] for step in results["steps"].values())
        ...
        self.logger.info(f"Total duration: {format_duration(results['total_duration_ms'])}")
        self.logger.info("=" * 60)
